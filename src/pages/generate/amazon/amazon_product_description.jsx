...
import AmazonProductDescription from '../../../components/Generate/Amazon/AmazonProductDescription';
import { useTranslation } from "react-i18next";
import { openSnackBar } from '../../../redux/snackBarReducer';
import { useDispatch, useSelector } from "react-redux";
import { generateAmazonProductDescription } from '../../../redux/template/amazon';
...
export default function AmazonProductDescriptionPage() {
  ...
  const generate = async (data, count, type, lang) => {
    if(!loading){
      ...
      if(is_valid){
        ...
        let res = await dispatch(generateAmazonProductDescription(sendData));
        if(res != false){
        ...
        <div className="generate_body" >
          <div className='generate_body_main'>
            <div className='generate_input border-gray-300 my-4 ml-8 bg-white' style={{borderWidth: "1px", borderRadius: '25px'}}>
              <Header 
                title={t("amazon_product_description")}
                content={t("amazon_product_description_content")}
              />
              ...
              />
              <Footer 
                type = "amazon_product_description"
